  ...
  void initState() {
    ...
    ever(_controller.isLoading, (isLoading) {
      if (isLoading == false) {
        ...
        // Load allMeals sau khi controller.onInit() hoàn thành
    ...
    // Lắng nghe thay đổi planExerciseCollection để tự động reload workouts
    ...
    // Lắng nghe thay đổi planMealCollection để tự động reload meals
    ...
    // Fallback: Load sau 1 giây nếu chưa load được
    Future.delayed(const Duration(seconds: 1), () {
      if (mounted) {
        // Kiểm tra lại workout plan
        if (_controller.currentWorkoutPlan.value == null) {
          _controller.loadDailyGoalCalories();
        }
        // Load allMeals
        if (allMeals.isEmpty && _controller.planMealCollection.isNotEmpty) {
          _controller.loadAllMealList().then((value) {
            if (mounted) {
              setState(() {
                allMeals = value;
  ...
  _handleSelectExercise(WorkoutCollection col) async {
    ...
    if (colSetting != null) {
      ...
      if (col.generatorIDs.isEmpty && col.id != null && col.id!.isNotEmpty) {
        print(
            '⚠️ _handleSelectExercise: generatorIDs rỗng, đang load lại planExercise cho collection ${col.id}');
        ...
        print(
            '✅ _handleSelectExercise: Đã load ${col.generatorIDs.length} generatorIDs');
      }
